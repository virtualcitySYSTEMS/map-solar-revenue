...
class VcSolarInteraction extends AbstractInteraction {
  ...
  _selectFeature(event: InteractionEvent): void {
    const { feature } = event;
    if (feature instanceof Cesium3DTileFeature) {
      if (this._localFeatureTrack.indexOf(feature.featureId) === -1) {
        const attributes = feature.getProperty('attributes');
        if (
          attributes.solarArea &&
          this._app.maps.activeMap instanceof CesiumMap
        ) {
          const camera = this._app.maps.activeMap.getScene()?.camera;
          const moduleFeature = reactive<SolarModule>({
            ...
            kwp: areaToKwp(
              this._vcSolarOptions.kwpPerArea,
              attributes.solarArea,
            ),
            solarIrradiation: calculateSolarIrradiation(feature),
          ...
          if (event.position && camera) {
            moduleFeature.screenshotInfo = {
              position: mercatorToCartesian(event.position),
              heading: camera.heading,
              pitch: camera.pitch,
              distance: attributes.solarArea,
            };
          }
